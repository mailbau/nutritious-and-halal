...
export default function SearchBar() {
  ...
  async function onCheck() {
    ...
    setLoading(true)
    try {
      const res = await axios.get(`${API}/api/check`, { params: { name: query } })
      setResult(res.data)
    } catch (error) {
              ...
              <div className="flex items-start gap-6">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    <h3 className="text-xl font-bold text-gray-900">{result.item.name}</h3>
                    <span className={`px-3 py-1 rounded-full text-sm font-semibold ${result.item.halal
